...
short **FrequencyApprox::normalize(float **keyStrokes, int nrows)
{
	...
	for (int i = 0; i < nrows; i++)
	{ //find maximum and applay weighting, here: sqrt
...
float *FrequencyApprox::frequencyToKeys(float *spectrum, int size, int sampleRate)
{
	...
	{ //only to nyquist f
		...
		for (j; j < NO_KEYS; j++)
		{
			if ((i + 1) * fres > fgu[j] && (i + 1) * fres <= fgo[j])
			{											 //frequency is in bin of key j
...
float FrequencyApprox::abs(kiss_fft_cpx c)
{ // returns abs from complex number
